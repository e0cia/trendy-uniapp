  ...
  <view class="template-help tn-safe-area-inset-bottom">
    ...
    <view class="tn-margin tn-padding-top-xl">
      <view class="tn-bg-gray--light tn-flex" style="border-radius: 10rpx;padding: 20rpx 30rpx;">
        ...
        <input placeholder="请输入转增用户" type="number" min="0" max="100000" v-model="userId"
               @input="click"></input>
      </view>
    </view>
    <view class="tn-margin tn-padding-top-xl">
    ...
    <view class="tn-margin kk-d-style tn-padding-top-xl">
      ...
      <p class="text1">2.本次转增未设定任何形式的对价</p>
      ...
      <view class="custom-modal-content">
        <view class="">
          ...
          <view class="tn-bg-gray--light"
            ...
            <input placeholder="请输入支付密码" v-model="payPassword"
...
export default {
  ...
  methods: {
    ...
    openModel() {
      ...
      if (util.isBlank(this.trandeNumber)) {
        this.$t.message.toast('转增数量不可以为空')
        return
      } else if (this.trandeNumber <= 0) {
        this.$t.message.toast('转增数量必须大于0')
        return
      }
